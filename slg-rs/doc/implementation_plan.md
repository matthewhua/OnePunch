...
**完成情况**：
- 已新增 `shared::battle` 纯计算模块，拆分为模型、伤害计算、技能、回合执行、战报、结果结算。
- 已实现兵种克制、速度决定行动顺序、主动/被动伤害技能、治疗技能、最大回合胜负判定。
- 已生成战斗损失统计（死亡/伤兵拆分）和可转换/编码为 protobuf `BattleReport` 的战报动作列表。
- 已补定向单元测试覆盖伤害克制、行动顺序、技能触发、超回合判定、战报 protobuf 编码。
