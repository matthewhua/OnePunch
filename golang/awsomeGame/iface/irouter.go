...
type IRouterSlices interface {
	// Use Add global components (添加全局组件)
	Use(Handlers ...RouterHandler)

	// AddHandler Add a route (添加业务处理器集合)
	AddHandler(msgId uint32, handlers ...RouterHandler)

	// Group Router group management （路由分组管理，并且会返回一个组管理器）
	Group(start, end uint32, Handlers ...RouterHandler) IGroupRouterSlices

	// GetHandlers Get the method set collection for processing （获得当前的所有注册在MsgId的处理器集合）
...
type IGroupRouterSlices interface {
	...
	// Add global routing components (添加业务处理器集合)
	AddHandler(MsgId uint32, Handlers ...RouterHandler)
}
