...
type MsgHandle struct {
	...
	// A collection of idle workers, used for conf.WorkerModeBind
	// 空闲worker集合，用于conf.WorkerModeBind
...
func newMsgHandle() *MsgHandle {
	...
	if conf.GlobalObject.WorkerMode == conf.WorkerModeBind {
		...
		conf.GlobalObject.WorkerPoolSize = uint32(conf.GlobalObject.MaxConn)
		freeWorkers = make(map[uint32]struct{}, conf.GlobalObject.WorkerPoolSize)
		for i := uint32(0); i < conf.GlobalObject.WorkerPoolSize; i++ {
...
// Intercept Data processing interceptor that is necessary by default in Zinx
// (默认必经的数据处理拦截器)
func (mh *MsgHandle) Intercept(chain iface.IChain) iface.IcResp {
...
// (立即以非阻塞方式处理消息)
func (mh *MsgHandle) doMsgHandler(request iface.IRequest, workerID int) {
	defer func() {
		if err := recover(); err != nil {
...
// (切片路由添加)
func (mh *MsgHandle) AddRouterSlices(msgId uint32, handler ...iface.RouterHandler) iface.IRouterSlices {
	mh.RouterSlices.AddHandler(msgId, handler...)
	return mh.RouterSlices
}

// Group routes into a group (路由分组)
func (mh *MsgHandle) Group(start, end uint32, Handlers ...iface.RouterHandler) iface.IGroupRouterSlices {
	return NewGroup(start, end, mh.RouterSlices, Handlers...)
}

func (mh *MsgHandle) Use(Handlers ...iface.RouterHandler) iface.IRouterSlices {
	mh.RouterSlices.Use(Handlers...)
	return mh.RouterSlices
}

func (mh *MsgHandle) doMsgHandlerSlices(request iface.IRequest, workerID int) {
	defer func() {
		if err := recover(); err != nil {
			zlog.Ins().ErrorF("workerID: %d doMsgHandler panic: %v", workerID, err)
		}
